...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 1, 'input_size': (2048,20), 'pool_size': None,
...
class PatchEmbed(nn.Module):
    ...
    """
    def __init__(self, img_size=20, patch_size=1, in_chans=2048, embed_dim=768):
        super().__init__()
        #img_size = (img_size)
...
class VisionTransformer(nn.Module):
    """ Vision Transformere
    """
    def __init__(self, img_size=20, patch_size=1, in_chans=2048, num_classes=1, embed_dim=768, depth=12,
    ...
    def forward_features(self, x):
        ...
        if x.size(1) != self.pos_embed.size(1):
            ...
            P = x.size(1)
            #other_pos_embed = other_pos_embed.reshape(1, x.size(2), P)
            new_pos_embed = F.interpolate(other_pos_embed, size=(W), mode='nearest')
...
class TimeSformer(nn.Module):
    def __init__(self, img_size=20, patch_size=1, num_classes=400, num_frames=8, attention_type='divided_space_time',  pretrained_model='', **kwargs):
